                      ...
                      {item.contacts ? (
                        <div className="grid grid-cols-[auto_auto] items-center gap-x-2 gap-y-2">
                          {item.contacts.map((contact) => (
                            <Fragment key={contact.country}>
                              <span className="text-[13.5px] leading-relaxed whitespace-nowrap text-white/85">
                                {contact.phone} ({contact.country})
                              </span>
                              <span className="flex items-center justify-self-start gap-1.5">
                                <a
                                  href={`tel:${contact.tel}`}
                                  aria-label={`Call ${contact.country} office`}
                                  className="flex h-7 w-7 items-center justify-center rounded-full bg-white/10 text-white/70 transition-colors hover:bg-maroon hover:text-white"
                                >
                                  <Icon paths={item.icon.paths} size={13} strokeWidth={1.8} />
                                </a>
                                <a
                                  href={contact.wa}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  aria-label={`WhatsApp ${contact.country} office`}
                                  className="flex h-7 w-7 items-center justify-center rounded-full bg-white/10 text-[#25D366] transition-colors hover:bg-[#25D366] hover:text-white"
                                >
